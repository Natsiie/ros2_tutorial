"""    
    ...
    流程：
        ...
        3.自定义节点类；
            3-1 创建客户端
            3-2 连接服务器（对于服务通信而言，如果客户端连接不到服务器就不能发送请求）
            3-3 发送请求
        4.调用spin函数,并传入节点对象；
            需要调用服务的函数，根据连接结果做下一步处理
            连接服务后，调用请求发送函数
            再处理响应结果
        5.资源释放。
...
class AddIntClient(Node):
    def __init__(self):
        super().__init__("add_int_client_node_py")
        self.get_logger().info("客户端创建成功！")
        #创建客户端
        self.client = self.create_client(AddInt,"add_ints")
        #连接服务器
